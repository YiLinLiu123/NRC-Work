...
```{r testing functions}
    ...
    ## shortened due to some origin &destination pairs are unreachable 
    ## by driving ex: https://maps.googleapis.com/maps/api/directions/json?origin=Wanham,Alberta&destination=BakerLake,Nunavut&key=AIzaSyBGTs-gZCbyP8n0Hvw_VZ76Z6YrST1DNa8
...
*   Internet recommanded to use [geosphere](https://cran.r-project.org/web/packages/geosphere/index.html) package
```{r}
    rm(list = ls())
    require(geosphere)
