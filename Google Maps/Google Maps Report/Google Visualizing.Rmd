...
```{r Visualization Example 2 Libraries, results="hide"}
 # Libraries:
    library(leaflet)
    library(httr)
    library(jsonlite)
...
```{r Visualization Example 2 Places_Of_Interest}
    ...
    # adding the loceations onto the map using google Icons
...
_GOAL: The goal is to map the number of crimes in Alabama  
as a percentage of population and display it
on the map.__


1:  Downloading and Saving Data:
```{r Visualitzation Example 3}
    rm(list = ls())
    require(httr)
    require(dplyr)
    require(leaflet)
...
```{r Visualization Example3: organizing Data}  
    org_Data = cbind(data[1:length,1:2],lat,
            lng)
    org_Data = as.data.frame(org_Data)
   
     ## Reorganizing data frame to chage class
...
```{r Visualization Example3: Map}
    ...
    ## the fraction.
    radius_Factor = 100000
    
     example3_Map = example3_Map %>%
         addCircles(lng=org_Data$lng,
             lat=org_Data$lat,
             popup=popup,
