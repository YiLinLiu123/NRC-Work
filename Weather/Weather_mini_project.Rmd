...
```{r lag columns}
    ...
    file_Path = "temperature-Copy.csv"
    ...
    savingLagData = function(data,start_Col=3, lag=1, lag_Increment=0,file_Path){
        ...
        write.table(x=data, file=file_Path, row.names=FALSE,
                    sep=",")
    }
    
    
    ##storing the two formated data tables:
    
    savingLagData(data = first, file_Path="first.csv",lag_Increment = 1)
    savingLagData(data = second, file_Path="second.csv",lag_Increment = 1)
   
```


2:  Getting Complete Data:
...
```{r data table}
    ...
    current=grep("current", names(formated))-1 ## subtract 1 because 
    ## date will be discarded later.
    ...
    ##  lower bound is not included, but upper is.
...
```{r Plotting Graphs}
    ...
    plot = ggplot(data=table, aes(x=strptime(table$Date,format="%Y-%m-%d %H:%M:%S"),
        ...
        geom_errorbar(aes(ymin=table$current,
                          ymax = table$maxPredict),size=table$erUpFreq/sizeFactor,
                      color = "blue")+
            geom_errorbar(aes(ymin=table$minPredict,
                          ymax = table$current), size=table$erDownFreq/sizeFactor,
                          color="blue")+
        ##  Trend Lines
        geom_smooth()
