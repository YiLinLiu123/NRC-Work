...
```{r}
    ...
    UnlistWithNA_Copy = function(field, list)
    {
        ...
        if(length(field)==1)
        {
            ...
            complete[notNull]= unlist(lapply(list, function(x) x[[field]]))
            
        }else if(length(field)==2)
        {
            ##similar logic as the above.
            notNull=unlist(lapply(list,function(x){!is.null(x[[field[1]]][[field[2]]])}))
            complete[notNull]= lapply(list,function(x) x[[field[1]]][[field[2]]]) 
            
        }
        if (length(field)==3){
		notnulls <- unlist(lapply(lst, function(x) 
			tryCatch(!is.null(x[[field[1]]][[field[2]]][[field[3]]]),error=function(e) FALSE)))
		complete <- rep(NA, length(lst))
		complete[notnulls] <- unlist(lapply(lst[notnulls], function(x) X = x[[field[1]]][[field[2]]][[field[3]]]))
        }
    ...
    post_Id = UnlistWithNA_Copy(c("id"), query_Data)
    place_Id = UnlistWithNA_Copy(c("place","id"),query_Data)
